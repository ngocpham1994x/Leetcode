...
public class P00000StudentHeights {

    public static void main(String[] args) {
        Integer[] arrInt = new Integer[]{5,4,3,6,1,7};

       int num = solution(arrInt);
    ...
    public static int solution(Integer[] A) {
        ...
        int rowCount=0;
        ArrayList listOfRow = new ArrayList();
        ...
        for (int i = 1; i < A.length; i++){
            ...
            for(int j = 0; j<rowCount; j++){
                ArrayList currentRow = (ArrayList) listOfRow.get(j);
                int lastPositionOfCurrentRow = currentRow.size()-1;
                if((int) A[i] < (int) currentRow.get(lastPositionOfCurrentRow)){
                    currentRow.add(A[i]);
                    isAdded = true;
                    break;
                }
...
All the students are asked to stand in rows. The students arrive by one, sequentially (as their heights appear in A).
