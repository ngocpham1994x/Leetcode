...
public class P00002LargestSubArrayLengthK {
    ...
    //basically doing sliding window twice. One for finding all sub-arrays, one for finding max array in sub-arrays list
    public static int[] findLargestSubArr(int[] arr, int k) {
        ...
        //setup so the ArrayList is not null
        for(int i=0; i<k; i++)
            maxArrBetweenTwoArr.add(0);


        //"sliding-window" of length 1, result in "size()" iteration
...
//            System.out.println(maxArrBetweenTwoArr);
    ...
    public static ArrayList findAllSubArr(int[] arr, int k) {
        ...
        //legit sliding-window with length of k, result in "length-k+1" iterations
...
//Extra practice on this easier problem P00239 from Leetcode's list of problems:
